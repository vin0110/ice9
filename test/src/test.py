...
def usage():
  ...
  -h, --help:		this message
          """
BASE_DIR=os.path.join(os.getenv("HOME"), "work","ice9")
TEST_DIR=os.path.join(BASE_DIR,"test")
COMPILER=os.path.join(BASE_DIR,"src","ice9")
...
def runtest(test, inoptions={}):
    ...
    if out:
        try:
            ...
            raise TestError('Could not open output file "%s"' % (out_,))
    else:
        out = False
        output = "/dev/null"
    ...
    if run:
        cmd = './%s %s >' % (test, config.get('args', ''),)
        cmd += output
        if Verbose > 1:
            print 'executing as:', cmd
        rc = os.system(cmd)
        if rc != config.get('status', 0):
            status.append('execution status failure')
            if Verbose > 1:
                print status[-1]

        if out and os.system('diff -q %s %s > /dev/null', (out_, output,)):
            status.append('execution output different')
            if Verbose > 1:
                print status[-1]
    if out:
        os.unlink(output)
